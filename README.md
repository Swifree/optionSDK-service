...
1. 登录FOTA券商 [管理后台](https://option-manager.fota.com)（账号在商务合作达成后由FOTA提供）
2. 申请测试环境apikey/apisecret，后续api使用上述秘钥签名，签名方式见 [apisecret签名](#apisecret签名)
3. 服务端在用户开通期权或者注册时，同步调用fota的api [新建券商子账号](#新建券商子账号)，fota将返回一个uid和password，后台需保存账号间的映射关系，后续登录时使用
4. 用户登录时使用上述接口返回的uid和password，调用[券商子账号登录](#券商子账号登录)，登录成功得到token，该token需要在客户端SDK设置，该token的默认有效期为7天
5. 资金的转入转出，具体见接口文档
6. 资金往来的流水查询，具体见接口文档
7. 交易记录的信息查询，具体见接口文档
...
- [资金转出](#资金转出)   
- [资金转入转出查询](#资金转入转出查询)  
- [资金转入转出分页查询](#资金转入转出分页查询) 开发中...
- [完整交易记录](#完整交易记录) 返回字段增加，尚未上线，以实际返回为准...
- [券商单个子账号余额查询](#券商单个子账号余额查询)
- [券商所有子账号余额查询](#券商所有子账号余额查询) 开发中...
- [券商支持币种查询](#券商支持币种查询) 开发中...
- [券商子账号查询](#券商子账号查询) 开发中...
- [公共错误码](#公共错误码)
